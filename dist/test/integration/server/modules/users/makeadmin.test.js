...
mocha_1.describe('GET /api/v1/users/makeadmin:id', () => {
    ...
    mocha_1.it('Should make a user admin', async () => {
        await app
            .get(`/api/v1/users/makeadmin/${user.id}`)
            .set('Authorization', adminToken)
            .expect(200);
        const updatedUser = await typeorm_1.getCustomRepository(user_1.default).findByUsername(user.username);
        chai_1.expect(updatedUser.role).eql(Authenticator_1.Role.admin);
    });
    mocha_1.it('Should return 403 not allowed error when not admin', async () => {
        await app
            .get(`/api/v1/users/makeadmin/${admin.id}`)
    ...
    mocha_1.it('Should return 401 when token is missing', async () => {
        const res = await app.get(`/api/v1/users/makeadmin/${user.id}`).expect(401);
        chai_1.expect(res.body.code).equals(30002);
    });
});
//# sourceMappingURL=makeadmin.test.js.map